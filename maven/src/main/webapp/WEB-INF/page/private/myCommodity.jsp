...
<script type="text/javascript">
	function history(userFlag) {
		$.jBox("iframe:${ctx}/salecommodityAction.do?method=toAddCommodity&flag="+userFlag, {
			title : "",
			width : 750,
			height : 550,
			buttons : { /*'关闭': true*/}
    ...
    <div class="pagebody1">
    	<div class="wid">
                <ul class="ht1">
                    ...
                    <li><button class="new1" onclick="history('${userFlag}');" style="cursor:pointer;">新增货架</button></li>
                </ul>
                <div class="clear"></div>
        </div>
        <div class="wid">  
        	...
        	<div class="wid2">
                    <ul id="demo">
                        <c:forEach var="sale" items="${salecommodityList_up}" >
                            <li  >
                                <dt onclick="<c:if test="${empty sale.title}">history('${userFlag}');</c:if><c:if test="${not empty sale.title}">productgroupedit('${sale.id}');</c:if>"><a href="#"><c:if test="${not empty sale.picurl}"><img src="${ctx}${sale.picurl}"/></c:if>
				...
				<div id="List1">
                    ...
                    <ul>
                        <c:forEach var="sale" items="${salecommodityList_down}" >
                              <li >
                                 <p   class="p1" onclick="<c:if test="${empty sale.title}">history('${userFlag}');</c:if><c:if test="${not empty sale.title}">productgroupedit('${sale.id}');</c:if>"><a href="javascript:;"><c:if test="${not empty sale.picurl}"><img src="${ctx}${sale.picurl}"/></c:if></a>
                                 
                                 
                                    <c:if test="${empty sale.picurl}"><img src="${ctx}/static/parts/images/tu007.png"/></c:if>
