          ...
          <div class="pagebody11_top"><!--输入用户名-->
          	<form id="searchStaffForm" name="searchStaffForm" action="${ctx}/staff/staffList" method="post">
                      ...
                      <table width="1000" border="0"  class="dsfd yuan" >
                          <c:forEach var="staff" items="${page.list}" varStatus="status">
                              <tr>
                                ...
                                <td width="80"><c:if test="${staff.roleId==4}">普通员工</c:if><c:if test="${staff.roleId==3}">管理员</c:if></td>
               ...
               <ul class="page">
					    ...
					    <a href="${ctx}/staff/staffList?pageNo=${pageNo}&pageSize=10" style="background: #C30D23 none repeat scroll 0% 0%;color:#ffffff;">${pageNo}</a>
					     </c:when>
					   <c:otherwise>
					   <a href="${ctx}/staff/staffList?pageNo=${pageNo}&pageSize=10" >${pageNo}</a>
					   </c:otherwise>
					   </c:choose>
